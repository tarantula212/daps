...
def rename_files(
    ...
    Args:
        matched_assets: Dictionary of matched logo assets.
            ...
            with progress(
                ...
                for item in pbar:
                    ...
                    for file in files:
                        ...
                        asset_name = (
                            re.search(r" - (Logo|SquareArt|Background)", file)
                            .group(1)
                            .lower()
                        )
                        if config.asset_folders:
                            new_file_name = f"{asset_name}{file_extension}"
                        else:
                            new_file_name = f"{folder}_{asset_name}{file_extension}"
...
def main(config: SimpleNamespace) -> None:
    """
    Entrypoint for asset_renamerr.py.
    Loads configuration, fetches media and assets, matches assets, performs renames,
    and optionally syncs to Google Drive.
    Args:
    ...
    try:
        ...
        if config.sync_assets:
            ...
            logger.debug("Sync assets is disabled. Skipping...")
        prefix_index = create_new_empty_index()
        logger.info("Gathering all the assets, please wait...")
        ...
        if matched_assets and any(matched_assets.values()):
            ...
            if any(output.values()):
                ...
                logger.info("No new asset to rename.")
        else:
